...
Respond with valid JSON matching the schema.`;
  ...
  const queryWithRetry = async (modelName: string) => {
    let attempt = 0;
    const maxRetries = 3; // Retry up to 3 times (total 4 attempts)
    ...
    while (true) {
      try {
        ...
        // We only retry on 503/500/429 or generally transient network errors.
        const status = err.status || err.statusCode;
        const isRetryable = !status || [500, 503, 429].includes(status) || err.message?.includes("503") || err.message?.includes("500") || err.message?.includes("429");
  ...
  try {
    ...
    console.error("Error generating trivia inside serverless function:", error);
    return res.status(500).json({ 
